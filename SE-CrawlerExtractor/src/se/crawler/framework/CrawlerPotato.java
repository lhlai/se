...
public class CrawlerPotato implements Runnable{
	...
	private LinkFilter filter = new PotatoLinkFilter();           //定义URL过滤器2
				    ...
				    if(content!=null){		    				    	
					    ...
					    if(d==1){ 	   									//首页时把所有导航页面的url放进队列
					    	...
					    	if("http://www.potato.agri.cn/"==tmpurl){					    		
					    		...
					    		System.out.println("中国马铃薯网共有: "+links.size()+" 个导航页面!");
					    		for(String link:links){
						            LinkQueue.addUnvisitedUrl(link,d+1); 	  			//所有导航页面入队
					            }
					    	}else{
					    		System.out.println("这是抓取中国马铃薯网的定制爬虫,您提供的种子url有误: "+tmpurl);
					    	}
				    	}else{				    		
				    		String urlType = "";

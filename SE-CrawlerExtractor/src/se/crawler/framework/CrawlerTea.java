...
public class CrawlerTea implements Runnable{
	...
	private LinkFilter filter = new TeaLinkFilter();           //定义URL过滤器2
				    ...
				    if(content!=null){		    				    	
					    ...
					    if(d==1){ 	   									//首页时把所有导航页面的url放进队列
					    	...
					    	if("http://www.tea.agri.cn/"==tmpurl){					    		
					    		...
					    		System.out.println("中国茶网共有: "+links.size()+" 个导航页面!");
					    		for(String link:links){
						            LinkQueue.addUnvisitedUrl(link,d+1); 	  			//所有导航页面入队
					            }
					    	}else{
					    		System.out.println("这是抓取中国茶网的定制爬虫,您提供的种子url有误: "+tmpurl);
					    	}
				    	}else{				    		
				    		String urlType = "";
	...
	private void saveToLocal(String urlType,String content, String url) {
		...
		if(urlType=="DetailPage"){
			if(url.contains("newscw")){     //新闻频道
				tmppath = "html/tea-newscw/DetailPage/";				
			}else if(url.contains("jspd")){  //技术频道
				tmppath = "html/tea-jspd/DetailPage/";
			}else if(url.contains("shichangcw")){  //经济频道
				tmppath = "html/tea-shichangcw/DetailPage/";
			}else if(url.contains("wenhuacw")){		//文化频道
				tmppath = "html/tea-wenhuacw/DetailPage/";
			}else if(url.contains("kejicw")){     	//品牌频道
				tmppath = "html/tea-kejicw/DetailPage/";
			}else if(url.contains("fuwu")){     	//国际频道
				tmppath = "html/tea-guoji/DetailPage/";
			}else if(url.contains("sppd/spzb")){     	//国际频道
				tmppath = "html/tea-shipin/DetailPage/";
			}else if(url.contains("gdcg")){     	//各大茶馆
			...
			}else if(url.contains("sppd/spzb")){     	//视频频道
